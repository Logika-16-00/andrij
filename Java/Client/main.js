...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const lobbiesDiv = document.querySelector(".multiplayer-items");
  const createBtn = document.getElementById("createLobbyBtn");

  if (!lobbiesDiv || !createBtn) {
...
createBtn.onclick = () => {
  ...
  document.querySelectorAll(".your-name").forEach(el => el.remove());
  // 🔥 сховати старий HUD Your name
  // 🔥 сховати старий HUD Your name
  if (yourNameHud) yourNameHud.style.display = 'none';
  ...
  // знайти та очистити весь multiplayer
  const multiplayerRoot = createBtn.closest(".menu--multiplayer");
  multiplayerRoot.innerHTML = "";

  const title = document.createElement("h1");
  title.textContent = "LOBBY";

  // ===== HOST TEXT =====
  const hostText = document.createElement("h2");
  hostText.textContent = "Host: " + playerName;

  // позиція зверху
  hostText.style.position = "fixed";
  hostText.style.top = "100px";
  hostText.style.left = "50%";
  hostText.style.transform = "translateX(-50%)";
  hostText.style.margin = "0";
  hostText.style.zIndex = "1000";

  // ===== YOUR NAME (лівий нижній кут) =====
  const yourName = document.createElement("h2");
  yourName.textContent = "Your name: " + playerName;
  yourName.style.position = "fixed";
  yourName.style.bottom = "20px";
  yourName.style.left = "20px";
  yourName.style.margin = "0";
  yourName.style.zIndex = "1000";
...
multiplayerRoot.appendChild(backBtn);        

  // додати все
  multiplayerRoot.appendChild(title);
  multiplayerRoot.appendChild(hostText);
  multiplayerRoot.appendChild(yourName);
  ...
  function renderLobbies(lobbies) {
    ...
    lobbies.forEach(lobby => {
      ...
      `;
        ...
        const multiplayerRoot = div.closest(".menu--multiplayer");
        if (!multiplayerRoot) return;
        ...
        const yourName = document.createElement("h2");
        yourName.textContent = "Your name: " + (state?.playerName || "Player");
        ...
        host.textContent = "Host: " + lobby.host;
        ...
        multiplayerRoot.appendChild(title);
        multiplayerRoot.appendChild(yourName);
        multiplayerRoot.appendChild(host);
  ...
  const RED =    { r: 0xff, g: 0x3b, b: 0x3b }; // новий червоний колір
  ...
  const MENU_NAME = Symbol('MENU_NAME');
  ...
  const MENU_SHOP = Symbol('MENU_SHOP'); // added shop menu symbol

  const MENU_MULTIPLAY = Symbol('MENU_MULTIPLAY'); // added multiplayer menu symbol
  ...
  const getTarget = (() => {
        ...
        return function getTarget() {
                ...
                let color = pickOne([BLUE, GREEN, ORANGE, RED]);
  ...
  const menuShopNode = $('.menu--shop'); // add this
  const menuMultiplayerNode = $('.menu--multiplayer'); // add this
  const menuNameNode = $('.menu--name');
  ...
  // --- HUD / Shop coin helpers (insert before renderMenus) ---
  function clearHUDText() {
    document.querySelectorAll('.score-lbl, .cube-count-lbl, .final-score-lbl, .high-score-lbl')
        .forEach(el => { if (el) el.textContent = ''; });
}
window.clearHUDText = clearHUDText;

function updateHudCoins() {
    const hudEl = document.querySelector('.hud .coin-lbl');
    const shopEl = document.querySelector('.menu--shop .shop-coin-lbl');
    if (hudEl) hudEl.textContent = `Coins: ${state.store.coins || 0}`;
    if (shopEl) shopEl.textContent = `Coins: ${state.store.coins || 0}`;
}
window.updateHudCoins = updateHudCoins;

// Show/hide coin labels: hud (in-game) and shop label
function setCoinVisibility(showHud, showShop) {
    const hudEl = document.querySelector('.hud .coin-lbl');
    const shopEl = document.querySelector('.menu--shop .shop-coin-lbl');
    if (hudEl) hudEl.style.display = showHud ? '' : 'none';
    if (shopEl) shopEl.style.display = showShop ? '' : 'none';
}
window.setCoinVisibility = setCoinVisibility;

// Ensure coins start hidden in HUD and only appear in shop by default
document.addEventListener('DOMContentLoaded', () => {
    setCoinVisibility(false, false);
    updateHudCoins();
});
  ...
  function renderMenus() {
    ...
    hideMenu(menuShopNode); // hide shop by default
    hideMenu(menuMultiplayerNode); // hide multiplayer by default
    hideMenu(document.querySelector('.menu--name'));


    switch (state.menus.active) {
        case MENU_MAIN:
            showMenu(menuMainNode);
            // hide all coin labels while on main
            setCoinVisibility(false, false);
            break;
        ...
        case MENU_SHOP:
            if (typeof clearHUDText === 'function') clearHUDText();
            updateHudCoins();
            showMenu(menuShopNode);
            // show coins only in shop
            setCoinVisibility(false, true);
            break;

        case MENU_MULTIPLAY:
            showMenu(menuMultiplayerNode);
            setCoinVisibility(false, false);
            break;

        case MENU_NAME:
            showMenu(menuNameNode);
            setCoinVisibility(false, false);
  ...
  handleClick($('.play-normal-btn'), () => {
        ...
        playerNameHud.style.display = 'none';

        yourNameHud.style.display = 'none';
  ...
  handleClick($('.menu-shop-btn'), () => setActiveMenu(MENU_SHOP));
  // Open Multiplayer from main menu
  handleClick($('.instructions-btn'), () => setActiveMenu(MENU_MULTIPLAY));
  // Back button from shop/multiplayer -> main menu
  document.querySelectorAll('.shop-back-btn').forEach(el => handleClick(el, () => setActiveMenu(MENU_MAIN)));

  // Name confirm button
  handleClick($('#nameConfirmBtn'), () => {
    const name = $('#playerNameInput').value.trim();
    if (name) {
      state.playerName = name;
      localStorage.setItem("playerName", name);
      playerNameHud.textContent = name;
      playerNameHud.style.display = 'block';
      setActiveMenu(MENU_MAIN);
    }
  });

  // Create lobby button
  handleClick($('#createLobbyBtn'), () => {
    socket.send(JSON.stringify({ type: "CREATE_LOBBY", name: state.playerName }));
  });
  ...
  handleClick($('.play-multiplayer-btn'), () => {
    ...
    if (state.playerName) {
        playerNameHud.style.display = 'block';
        yourNameHud.style.display = 'block';
    }
  });
  ...
  function setActiveMenu(menu) {
        ...
        if (menu === MENU_MAIN) {
            playerNameHud.style.display = 'none';
            yourNameHud.style.display = 'none';
        } else if (menu === MENU_MULTIPLAY) {
            if (state.playerName) {
                playerNameHud.style.display = 'block';
                yourNameHud.style.display = 'block';
            }
            // Reset multiplayer menu to normal state
            $('.menu--multiplayer h1').textContent = "MULTIPLAYER";
            $('.multiplayer-items').style.display = 'flex';
            $('#createLobbyBtn').style.display = 'block';
            $('.your-name').style.display = 'block';
        }
...
const lobbyMenu = document.querySelector(".menu--lobby"); // якщо є окреме меню лобі
const multiplayerMenu = document.querySelector(".menu--multiplayer");

leaveLobbyBtn.addEventListener("click", () => {
    // якщо є підключення до лобі — тут можна вийти з нього
    // leaveLobby(); або socket.disconnect(); (якщо є)
